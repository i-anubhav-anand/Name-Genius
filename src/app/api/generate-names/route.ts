...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log("Generating names for:", { namingType, industry, traits: traits.length })
...
2. A brief meaning or explanation (2-3 sentences)
...
`
    ...
    // Set up a timeout for the OpenAI API call
    const timeoutPromise = new Promise((_, reject) => {
      setTimeout(() => reject(new Error("OpenAI API call timed out")), 20000);
    });

    // Use a faster model and set a lower response token limit
